...
const translations: Record<string, string> = {
  ...
  "Master the foundation":"Kuasai fondasi", "Build leverage":"Bangun leverage", "Turn value into impact":"Ubah nilai menjadi dampak",
  ...
  "Finance depth":"Kedalaman finance", "Technology + AI":"Teknologi + AI", "Communication + audience":"Komunikasi + audiens",
  "A finance career in progress.":"Perjalanan karier finance yang terus berkembang.", "Finance authority":"Otoritas finance", "Business + technology":"Bisnis + teknologi", "Visible body of work":"Jejak karya yang terlihat",
  "Only explicitly published projections appear here. Private and draft work remains in the Personal OS.":"Hanya proyek yang secara eksplisit dipublikasikan yang tampil di sini. Data privat dan draft tetap berada di Personal OS.",
  "Have a project that could benefit from this skill stack?":"Punya proyek yang bisa mendapatkan manfaat dari kapabilitas ini?",
  ...
  "Professional authority":"Otoritas profesional","Personal brand":"Personal brand","Short-form ideas":"Ide short-form","Deep-dive content":"Konten mendalam",
  "The public portfolio is one layer of a wider body of work. Follow the channels where I share ideas, experiments, lessons and new projects.":"Portofolio publik adalah salah satu lapisan dari keseluruhan karya. Ikuti kanal tempat saya membagikan ide, eksperimen, pelajaran, dan proyek baru.",
  ...
  "General WhatsApp":"WhatsApp Umum", "Contact page":"Halaman Kontak", "Back to Public Profile":"Kembali ke Profil Publik", "Public profile":"Profil publik",
  "Command your next move.":"Kendalikan langkah berikutnya.", "One operating system.":"Satu operating system.", "Operating loop":"Operating loop", "Quick Capture":"Quick Capture",
};
...
const reverse = Object.fromEntries(Object.entries(translations).map(([en,id]) => [id,en]));

function applyLanguage(lang: "en" | "id") {
  document.documentElement.lang = lang;
  document.documentElement.dir = "ltr";
  document.querySelectorAll<HTMLElement>("[data-i18n]").forEach((node) => {
    const entry = node.dataset.i18n ? keyTranslations[node.dataset.i18n] : undefined;
    if (entry) node.textContent = entry[lang === "en" ? 0 : 1];
  });
  const map = lang === "id" ? translations : reverse;
  document.querySelectorAll<HTMLElement>("body *").forEach((node) => {
    ...
    else if (lang === "en") node.textContent = original;
  });
}
...
export function LanguageSwitcher() {
  ...
  useEffect(() => {
    ...
    const next: "en" | "id" = saved === "id" ? "id" : "en";
    ...
    observerRef.current = new MutationObserver(() => {
      if (queued) return;
      queued = true;
      queueMicrotask(() => { queued = false; applyLanguage(langRef.current); });
    });
    if (document.body) observerRef.current.observe(document.body, { childList: true, subtree: true });
    return () => observerRef.current?.disconnect();
  }, []);

  useEffect(() => { langRef.current = lang; applyLanguage(lang); }, [lang]);
