...
import { ArrowLeft, ArrowUpRight, ExternalLink, MessageCircle, ShieldCheck } from "lucide-react";
...
export default async function ProjectDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (published) {
    const payload = (published.public_payload || {}) as { kind?:string; summary?:string; problem?:string; objective?:string; approach?:string; tools?:string; outcome?:string; lessons?:string; repositoryUrl?:string };
    return <main className="section section-white"><div className="container">
      ...
      <div className="editorial-grid">
        <article className="card" style={{borderRadius:24,minHeight:320}}><div className="tag">Case study</div><h2 style={{margin:"18px 0 10px",fontSize:30}}>What was done</h2><p>{payload.approach || payload.objective || "The published record does not include a detailed methodology yet."}</p>{payload.repositoryUrl&&<p style={{marginTop:22}}><a className="os-module-link" href={payload.repositoryUrl} target="_blank" rel="noreferrer">Open source repository <ExternalLink size={14}/></a></p>}<div className="pill-row" style={{marginTop:28}}><span className="pill"><ShieldCheck size={13}/> Explicitly published</span><span className="pill">Evidence-backed</span></div></article>
